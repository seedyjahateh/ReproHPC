process MRI_BATCH {
    ...
    input:
    tuple val(batch_id), val(spec_b64), val(expected_files), path(images, stageAs: 'image????.nii.gz')
    output:
    tuple val(batch_id), path(expected_files), path('result/task.json')
    script:
    """
    python -m reprohpc.task mri --spec-b64 '${spec_b64}' --output result image*.nii.gz
    """
}
